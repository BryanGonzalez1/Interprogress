...
<body>
  ...
  include_once "../sql/op_administrador.php";
    ...
    <div class="contenedor">
        ...
        ?>
        <div class="titulo">
            <h1>Revisar evidencia de dependencias</h1>
        </div>
        <div class="contenedor_form">
                ...
                <?php
                $consultaCant = "SELECT DISTINCT Dependencia_responsable FROM respuesta WHERE Estado_cuestionario = 3 AND Codigo_fecha = ".$fechas[0]." ";
                $conn2 = conectar();
                $resultado3 = mysqli_query($conn2, $consultaCant);
                ?>
                <div class="info_semaforo" style="width: 360px; margin-bottom:15px;">
                    <h2 class="h2semaforo" style="font-size: 18px;">🧑 Numero de dependencias: <?php echo mysqli_num_rows($resultado3);  desconectar($conn2); ?></h2>
                </div>
                ...
                <table style="margin-top: 25px;">
                    ...
                    if (!empty($_POST["btnbuscar"]) && !empty($_POST["caja_de_busqueda"])) {
                        if($_POST["caja_de_busqueda"] !== ""){
                            $consulta = "SELECT us.Correo, us.Nombre, us.Apellidos, dep.Nombre
                            FROM usuarios AS us  
                            INNER JOIN Dependencia AS dep  
                            ON us.Dependencia = dep.Codigo  
                            WHERE us.Correo LIKE '%" . $_POST["caja_de_busqueda"] . "%' OR us.Nombre LIKE '%" . $_POST["caja_de_busqueda"] . "%' OR us.Apellidos LIKE '%" . $_POST["caja_de_busqueda"] . "%' OR dep.Nombre LIKE '%" . $_POST["caja_de_busqueda"] . "%'";
                            $conn = conectar();
                            $resultado = mysqli_query($conn, $consulta);
                        }else{
                            $consulta = "SELECT usuarios.Correo,usuarios.Nombre,usuarios.Apellidos,Dependencia.Nombre FROM usuarios,dependencia WHERE usuarios.Dependencia = Dependencia.Codigo AND Rol = '4';";
                            $conn = conectar();
                            $resultado = mysqli_query($conn, $consulta);
                        }
                    ...
                    ?>   
                        <tr class="<?php echo $est; ?>">
                            ...
                            <td>
                                <a href="/administrador/menu_revcuestionario.php?dependencia=<?php echo $fila[3]; ?>&responsable=<?php echo$fila[0]; ?>&fecha=<?php echo $fechas[0]; ?>">
